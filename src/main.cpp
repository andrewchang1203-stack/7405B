...
void opcontrol() {
	...
	pros::MotorGroup left_mg({10, 16});    // Creates a motor group with forwards ports 1 & 3 and reversed port 2
	pros::MotorGroup right_mg({7, 19});  // Creates a motor group with forwards port 5 and reversed ports 4 & 6
	
	while (true) {
		int dir = master.get_analog(ANALOG_LEFT_Y);    // Gets amount forward/backward from left joystick
		int turn = master.get_analog(ANALOG_RIGHT_X);  // Gets the turn left/right from right joystick
		left_motor_group.move(dir + turn);
		right_motor_group.move(dir - turn);
